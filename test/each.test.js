...
describe('each()', () => {
  ...
  it('iterates every property of an object, passing the value, the corresponding key, and the entire object to the callback when a key has multiple values', () => {
    const triplets = {
      name: (
        'john',
        'tim',
    ...
    let count = 0;
    _.each(triplets, function(value, key, iteratedObj) {
      expect(value).toEqual(iteratedObj[key]);
      count += 1;
    });
    expect(count).toBe(3);
